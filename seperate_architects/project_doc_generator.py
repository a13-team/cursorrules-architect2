...
class FileChangeHandler(FileSystemEventHandler):
    ...
    def __init__(self, root_path: Path, cursorrules_path: Path, exclude_dirs: set[str]):
        ...
        # Add storage for file comments
        self.file_comments = {}
        # Load existing comments on initialization
        self.load_existing_comments()
        self.update_cursorrules()

    def load_existing_comments(self):
        """Load existing comments from the .cursorrules file."""
        if self.cursorrules_path.exists():
            ...
            if start_idx != -1 and end_idx != -1:
                tree_section = content[start_idx:end_idx]
                # Find lines with comments
                for line in tree_section.split('\n'):
                    if '#' in line:
                        file_part, comment = line.split('#', 1)
                        # Extract filename from the tree line
                        if "📄" in file_part:
                            filename = file_part.split("📄")[-1].strip()
                            self.file_comments[filename] = f"#{comment}"
    ...
    def update_cursorrules(self):
        ...
        for line in tree_lines:
            ...
            if "📄" in line:
                ...
                if filename in self.file_comments:
                    modified_line = f"{line} {self.file_comments[filename]}"
